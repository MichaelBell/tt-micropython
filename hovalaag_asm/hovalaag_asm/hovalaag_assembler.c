#define STB_DEFINE
#include "stb_reduced.h"  // http://github.com/nothings/stb
...
#else
#define fatal(...) { char err[400]; sprintf(err, __VA_ARGS__); mp_raise_ValueError(err); }
...
#if 0
static char *a_op[] = { "     " , "A=D, " , "A=M, " , "A=I1," , "A=I2," };
static char *b_op[] = { "    "  , "B=A,"  , "B=M,"  , "B=K,"            };
static char *c_op[] = { "      ", "C=M,  ", "DEC,  ", "DECNZ,",         };
static char *d_op[] = { "    "  , "D=A,"                                };
static char *w_op[] = { "    "  , "W=A,"  , "W=M,"  , "W=K,"            };
static char *o_op[] = { "     " , "O1=W," , "O2=W,"                     };
static char *f_op[] = { "       ", "F=Z(M),", "F=N(M),", "F=P(M),"      };
static char *j_op[] = { "     " , "JMP, " , "JMPT," , "JMPF,"           };
static char *m_op[] = { "M:0,    ", "M:-A,   ", "M:B,    ", "M:C,    ", "M:A>>1, ", "M:A+B,  ", "M:B-A,  ", "M:A+B+F,", "M:B-A-F,", "M:A|B,  ", "M:A&B,  ", "M:A^B,  ", "M:~A,   "};
#endif
...

      {
         ...
         if (units[10] >= 0 && has_constant && units[10] != units[9]) {
            if (units[10] >= 63 || units[9] < -32 || units[9] > 31)
               fatal("ASM error, line %d: internal error detecting constant mismatch", line_number);
...
static vls_instruction program[MAX_INSTRUCTIONS];
static char *label[MAX_INSTRUCTIONS];

uint32_t hovalaag_program[MAX_INSTRUCTIONS];
int hovalaag_num_instructions=0;
char **hovalaag_labels=label;
...
void vls_assemble(char **lines, int len)
{
   ...
   printf("Stripped comments\n");

   // assign addresses to labels
   pc = 0;
   for (i=0; i < len; ++i) {
      printf("Here %s\n", lines[i]);
      if (lines[i][0] != 0 && !isspace((int)lines[i][0])) {
   ...
   for (i=0; i < len; ++i) {
      ...
      if (s[0] != 0) {
         ...
         if (hovalaag_num_instructions == MAX_INSTRUCTIONS) fatal("Program too long");
         for (j=0; j < num_ins;) {
         ...
         program[hovalaag_num_instructions++] = vls_assemble_instruction(tokens, num_ins, i+1);
         free(old_tokens);
      }
      printf("Assembled line %d\n", i);
   }

   if (hovalaag_num_instructions == 0)
      fatal("ASM error: program was empty\n");

   stb_sdict_delete(labels);
   ...
   #if 1
   {
      ...
      int i;
      for (i=0; i < hovalaag_num_instructions; ++i) {
         ...
         hovalaag_program[i] = (alu << 28) | (a<<26) | (b<<24) | (c<<22) | (d<<21) | (w<<19) | (f<<17) | (j<<15) | (o<<14) | (io<<13) | (x<<12) | (k<<6) | l;
         //fwrite(&ins, 4, 1, ff);
      }
      //fclose(ff);
   }
